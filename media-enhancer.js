...
.agh-video-badge:before{content:'▶';color:#b8ff3d}.agh-news-video{width:100%;max-height:68vh;background:#000;border-radius:22px;display:block;object-fit:contain}.agh-video-copy{color:var(--muted,#9aa7a1);line-height:1.9}.agh-video-actions{display:flex;gap:8px;margin-top:14px}.agh-video-actions button{border:1px solid var(--line,#26372f);background:var(--panel,#111713);color:#fff;border-radius:13px;padding:10px 13px}
</style>`)}
function decorate(){document.querySelectorAll('[data-news]').forEach(card=>{const n=videoNews.get(card.dataset.news);if(!n||!n.video_url||card.querySelector('.agh-video-badge'))return;const badge=document.createElement('span');badge.className='agh-video-badge';badge.textContent='فيديو';card.appendChild(badge)})}
