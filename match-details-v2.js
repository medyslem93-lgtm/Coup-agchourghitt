(() => {
  ...
  const img = (u, a = "") =>
    `<img src="${esc(
      !u
        ...
        : /^(https?:|data:|blob:)/i.test(u)
          ? u
          : String(u)
              .replace(/^\.\.\//, "")
              .replace(/^\.\//, ""),
    )}" alt="${esc(a)}" onerror="this.onerror=null;this.src='assets/tournament.jpg'">`;
  const fmt = (d) => {
    if (!d) return "التاريخ غير محدد";
    try {
  ...
  async function open(id) {
    ...
    try {
      ...
      const pmap = new Map(players.map((p) => [p.id, p])),
        pn = (e) => e.player_name || pmap.get(e.player_id)?.name || "",
        teamName = (tid) =>
          tid === m.team_a_id
            ? m.team_a.name
            : tid === m.team_b_id
              ...
              : "";
      const localRef = (role) =>
        refs.find((r) => r.match_id === id && r.role === role) ||
        refs.find(
          (r) => !r.match_id && r.category === m.category && r.role === role,
        );
      const localRefs = (role) => {
        const own = refs.filter((r) => r.match_id === id && r.role === role);
        return own.length
          ? own
          : refs.filter(
              (r) =>
                !r.match_id && r.category === m.category && r.role === role,
            );
      };
      const assistants = localRefs("assistant");
      const score =
        m.status === "انتهت" || m.status === "مباشر"
          ? `${m.score_b ?? 0} - ${m.score_a ?? 0}`
          : "VS";
      ...
      console.error(e);
      show(
        `<div class="sheet-head"><button data-v2-close>إغلاق</button></div><div class="empty card"><b>تعذر تحميل تفاصيل المباراة</b><span>${esc(e.message || "حاول مرة أخرى")}</span></div>`,
      );
    }
  }

  function motm(events, pn, tn) {
    const e = events.find((x) => x.type === "رجل المباراة");
    return e
      ? `<div class="card motm"><b>رجل المباراة</b><span>${esc(pn(e))}${tn(e.team_id) ? " · " + esc(tn(e.team_id)) : ""}</span></div>`
      : "";
  }
  function eventRow(e, pn, tn, pmap) {
    const icons = {
        هدف: "⚽",
        "ركلة جزاء مسجلة": "⚽",
        "هدف عكسي": "⚽",
      ...
      assist = e.assist_name || pmap.get(e.assist_player_id)?.name || "";
    return `<div class="event-row"><span>${icons[e.type] || "•"}</span><div><b>${esc(e.type)}${e.minute != null ? " · " + e.minute + "'" : ""}</b><small>${esc(pn(e) || e.note || "")}${tn(e.team_id) ? " · " + esc(tn(e.team_id)) : ""}${assist ? " · صناعة: " + esc(assist) : ""}</small></div></div>`;
  }
  ...
  function getStartingPlayers(teamId, lineup, members, pmap) {
    if (!lineup) return [];
    return members
      .filter((x) => x.lineup_id === lineup.id && x.role === "أساسي")
      .sort((a, b) => (a.sort_order ?? 999) - (b.sort_order ?? 999))
      .map((x) => pmap.get(x.player_id))
      .filter(Boolean);
  }
  function getBenchPlayers(teamId, starters, players) {
    const ids = new Set(starters.map((p) => p.id));
    return players
      .filter((p) => p.team_id === teamId && !ids.has(p.id))
      .sort((a, b) =>
        String(a.name || "").localeCompare(String(b.name || ""), "ar"),
      );
  }
  function shortName(name = "") {
    const s = String(name).trim().split(/\s+/);
    return s.length <= 2 ? name : s.slice(0, 2).join(" ");
  }
  function teamTheme(name = "") {
    if (name.includes("الهلال")) return "hilal";
    if (name.includes("الشهيد")) return "martyr";
    return "default";
  }
  function getPlayerMatchEvents(player, events) {
    ...
    return events.filter(
      (e) =>
        ...
        (!e.player_id && String(e.player_name || "").trim() === n),
    );
  }
  function eventBadges(player, events) {
    const ev = getPlayerMatchEvents(player, events),
      goals = ev.filter((e) =>
        ["هدف", "ركلة جزاء مسجلة"].includes(e.type),
      ).length,
      assists = ev.filter((e) => e.type === "تمريرة حاسمة").length,
      y = ev.some((e) => e.type === "بطاقة صفراء"),
      r = ev.some((e) => e.type === "بطاقة حمراء");
  ...
  function renderFormationPitch(team, lineup, starters, events) {
    if (!lineup || !starters.length)
      return `<div class="empty card"><b>${esc(team.name)}</b><span>لم يتم تسجيل التشكيلة بعد.</span></div>`;
    const gk =
        starters.find((p) => p.id === lineup.goalkeeper_player_id) ||
        starters[0],
      field = starters.filter((p) => p.id !== gk.id),
      rows = formationToRows(lineup.formation, field.length);
    let cursor = 0;
    const rowHtml = rows
      .map((count, i) => {
        const arr = field.slice(cursor, cursor + count);
        cursor += count;
        const bottom =
          rows.length === 1 ? 52 : 24 + i * (60 / Math.max(rows.length - 1, 1));
        return `<div class="pitch-line" style="bottom:${bottom}%">${arr.map((p) => renderPitchPlayer(p, lineup, events)).join("")}</div>`;
      })
      .join("");
  ...
  function renderTeamPanel(
    ...
    const starters = getStartingPlayers(team.id, lineup, members, pmap),
      bench = getBenchPlayers(team.id, starters, players);
  ...
  function renderTeamSelector(m) {
    return `<div class="lineup-selector"><button class="active" data-lineup-team="${esc(m.team_a.id)}">${img(m.team_a.logo_url, m.team_a.name)}<span>${esc(m.team_a.name)}</span></button><button data-lineup-team="${esc(m.team_b.id)}">${img(m.team_b.logo_url, m.team_b.name)}<span>${esc(m.team_b.name)}</span></button></div>`;
  }
  function renderLineups(m, lineups, members, pmap, players, events) {
    const la = lineups.find((x) => x.team_id === m.team_a.id),
      lb = lineups.find((x) => x.team_id === m.team_b.id);
  ...
  function showPlayerCard(id) {
    ...
    if (!p) return;
    const team = p.team_id === ctx.m.team_a.id ? ctx.m.team_a : ctx.m.team_b,
      lineup = ctx.lineups.find((x) => x.team_id === p.team_id),
      ev = getPlayerMatchEvents(p, ctx.events),
      goals = ev.filter((e) =>
        ["هدف", "ركلة جزاء مسجلة"].includes(e.type),
      ).length,
      assists = ev.filter((e) => e.type === "تمريرة حاسمة").length,
      y = ev.filter((e) => e.type === "بطاقة صفراء").length,
      r = ev.filter((e) => e.type === "بطاقة حمراء").length,
      box = $("v2-player-card");
    if (!box) return;
    box.hidden = false;
    box.innerHTML = `<div class="player-card"><button data-player-card-close>×</button><div class="player-card-head">${p.photo_url ? img(p.photo_url, p.name) : '<div class="player-avatar">⚽</div>'}<div><b>${esc(p.name)}</b><span>${esc(team.name)}</span></div></div><div class="player-meta"><span><b>${p.number ?? "–"}</b>الرقم</span><span><b>${esc(p.position || "–")}</b>المركز</span><span><b>${lineup?.captain_player_id === p.id ? "نعم" : "لا"}</b>القائد</span><span><b>${lineup?.goalkeeper_player_id === p.id ? "نعم" : "لا"}</b>حارس</span></div><div class="player-events"><span>⚽ ${goals}</span><span>🎯 ${assists}</span><span>🟨 ${y}</span><span>🟥 ${r}</span></div></div>`;
  }

  function statsBlock(s, m, events) {
    const count = (type, tid) =>
      events.filter((e) => e.type === type && e.team_id === tid).length;
    const assistsA = count("تمريرة حاسمة", m.team_a_id),
      assistsB = count("تمريرة حاسمة", m.team_b_id);
    const yellowA = s?.yellow_cards_a ?? count("بطاقة صفراء", m.team_a_id),
      yellowB = s?.yellow_cards_b ?? count("بطاقة صفراء", m.team_b_id);
    const redA = s?.red_cards_a ?? count("بطاقة حمراء", m.team_a_id),
      redB = s?.red_cards_b ?? count("بطاقة حمراء", m.team_b_id);
    const official = [
      ["الأهداف", m.score_a ?? 0, m.score_b ?? 0, ""],
      ["التمريرات الحاسمة", assistsA, assistsB, ""],
      ["البطاقات الصفراء", yellowA, yellowB, ""],
      ["البطاقات الحمراء", redA, redB, ""],
    ];
    const recorded = [
      ["الاستحواذ", "possession", "%"],
      ["التسديدات", "shots", ""],
      ["على المرمى", "shots_on_target", ""],
      ["الركنيات", "corners", ""],
      ["الأخطاء", "fouls", ""],
    ]
      .filter(([, k]) => s && (s[k + "_a"] != null || s[k + "_b"] != null))
      .map(([label, k, u]) => [label, s[k + "_a"], s[k + "_b"], u]);
    const rows = [...official, ...recorded];
    return `<div class="card list-card stats-card-v2"><div class="stats-header"><b>${esc(m.team_a.name)}</b><span>الإحصائيات</span><b>${esc(m.team_b.name)}</b></div>${rows
      .map(([label, a, b, u]) => {
        const av = Math.max(0, Number(a) || 0),
          bv = Math.max(0, Number(b) || 0),
          total = av + bv || 1,
          ap = Math.round((av / total) * 100),
          bp = 100 - ap;
        return `<div class="stats-compare"><div class="stats-values"><strong>${a ?? "–"}${a != null ? u : ""}</strong><span>${esc(label)}</span><strong>${b ?? "–"}${b != null ? u : ""}</strong></div><div class="stats-bars" style="--a:${ap}%;--b:${bp}%"><i></i><i></i></div></div>`;
      })
      .join("")}</div>`;
  }
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      if (m) {
        ...
        open(m.dataset.openMatch);
        return;
      }
      if (e.target.closest("[data-v2-close]")) {
        e.preventDefault();
        e.stopImmediatePropagation();
        close();
        return;
      }
      const p = e.target.closest("[data-v2-pane]");
      if (p) {
        e.preventDefault();
        e.stopImmediatePropagation();
        document
          .querySelectorAll("[data-v2-pane]")
          .forEach((x) => x.classList.toggle("active", x === p));
        ["overview", "events", "lineups", "stats"].forEach((x) =>
          document
            .getElementById("v2-" + x)
            ?.classList.toggle("active", x === p.dataset.v2Pane),
        );
        return;
      }
      const ts = e.target.closest("[data-lineup-team]");
      if (ts) {
        e.preventDefault();
        document
          .querySelectorAll("[data-lineup-team]")
          .forEach((x) => x.classList.toggle("active", x === ts));
        document
          .querySelectorAll("[data-lineup-panel]")
          .forEach((x) =>
            x.classList.toggle(
              "active",
              x.dataset.lineupPanel === ts.dataset.lineupTeam,
            ),
          );
        return;
      }
      const pl = e.target.closest("[data-v2-player]");
      if (pl) {
        e.preventDefault();
        showPlayerCard(pl.dataset.v2Player);
        return;
      }
      if (e.target.closest("[data-player-card-close]")) {
        const box = $("v2-player-card");
        if (box) {
          box.hidden = true;
          box.innerHTML = "";
        }
