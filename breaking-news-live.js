...
function apply(row){
 ...
 const enabled=row?.breaking_news_enabled!==false && !!text && /^#\/?news(?:\/|$)/.test(location.hash);
 ticker.hidden=!enabled;
 ticker.style.display=enabled?'flex':'none';
 if(moving&&enabled)moving.textContent=text;
}
async function load(){const {data,error}=await db.from('site_settings').select('breaking_news_text,breaking_news_enabled').eq('id','main').maybeSingle();apply(error?null:data)}
load();
