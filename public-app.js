  ...
  function renderHome() {
    ...
    main.innerHTML = `<div class="page-shell">
      ...
      <section class="hero-layout">
        <div class="hero-copy-card agh-home-hero"><video class="agh-home-hero-video" autoplay muted loop playsinline preload="metadata" poster="assets/agh-cup-hero-poster-2026.jpg" aria-hidden="true"><source src="assets/agh-cup-hero-2026.mp4" type="video/mp4"></video><div class="agh-home-hero-copy"><span class="eyebrow">البطولة الرسمية · موسم ${escapeHtml(tournament.season || "2026")}</span><h1>${state.settings.hero_title ? escapeHtml(state.settings.hero_title) : "كأس أغشوركيت<br><em>2026</em>"}</h1><p>${escapeHtml(state.settings.hero_subtitle || "من لحظة البداية حتى التتويج: المباريات، الأندية، اللاعبون وكل أحداث البطولة في مكان واحد.")}</p><div class="hero-actions"><button class="primary-button" type="button" data-route="matches">تابع المباريات ${icon("arrow", "button-icon")}</button><button class="secondary-button" type="button" data-route="tournaments">استكشف البطولات</button></div></div><div class="agh-home-trophy" aria-hidden="true"></div><div class="agh-home-hero-foot"><span>⚽ ${matches.length} مباراة</span><span>🛡 ${tournamentTeams(tournament.id).length} فريق</span><span>● ${escapeHtml(tournament.short_name)}</span></div></div>
