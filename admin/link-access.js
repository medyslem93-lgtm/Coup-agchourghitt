(() => {
  ...
  if(!token){window.AGCH_ADMIN_LINK_ACTIVE=false;location.replace('login.html');return}
  ...
  window.supabase.createClient=(supabaseUrl,supabaseKey,options={})=>{
    ...
    headers['x-agh-admin-link']=token;
    ...
    client.auth.getSession=async()=>({data:{session:{user:{email:'الدخول عبر رابط الإدارة الخاص'}}},error:null});
    client.auth.signOut=async()=>{localStorage.removeItem(STORAGE_KEY);return {error:null}};
    return client;
  };
