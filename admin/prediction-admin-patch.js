(() => {
  ...
  let busy=false,lastKey='',queued=false;
  async function refresh(force=false){
    ...
    const key=sel.value+'|'+(document.getElementById('paCategory')?.value||'');if(!force&&key===lastKey&&sec.querySelector('.pa-selected-metrics'))return;busy=true;
    try{
      ...
      const html=`<div><b>${rows.length}</b><span>إجمالي توقعات المباراة</span></div><div class="good"><b>${correct}</b><span>التوقعات الصحيحة</span></div><div class="bad"><b>${wrong}</b><span>التوقعات الخاطئة</span></div><div class="wait"><b>${pending}</b><span>قيد انتظار النتيجة</span></div>`;
      if(box.innerHTML!==html)box.innerHTML=html;lastKey=key;
    }finally{busy=false}
  }
  function patchLabels(){const tab=document.querySelector('[data-tab="predictionAdmin"]');if(tab&&tab.textContent!=='توقعات المباريات')tab.textContent='توقعات المباريات';const h=document.querySelector('#predictionAdmin h2');if(h&&h.textContent!=='إدارة التوقعات والنقاط')h.textContent='إدارة التوقعات والنقاط'}
  const apply=()=>{queued=false;patchLabels();refresh(false)};
  const obs=new MutationObserver(()=>{if(!queued){queued=true;requestAnimationFrame(apply)}});
  const start=()=>{obs.observe(document.body,{childList:true,subtree:true});document.addEventListener('change',e=>{if(e.target?.id==='paMatch'||e.target?.id==='paCategory'){lastKey='';setTimeout(()=>refresh(true),80)}});document.addEventListener('click',e=>{if(e.target?.id==='paRefresh'||e.target?.id==='paSaveSettings'||e.target?.id==='paRecalc'||e.target?.closest?.('[data-pa-delete]')){lastKey='';setTimeout(()=>refresh(true),500)}});apply()};
