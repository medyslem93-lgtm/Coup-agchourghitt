(() => {
  ...
  function renderTeamsDirectory(active = 'all') {
    ...
    const body = `${tournamentChips(active)}${groups.map(([t,list]) => `<section class="agh-dir-section"><div class="agh-dir-section-title"><div>${img(t.logo_url,t.short_name || t.name)}<span><b>${esc(t.short_name || t.name)}</b><small>${list.length} فرق</small></span></div><button type="button" data-route="tournament/${esc(t.slug)}/teams">صفحة البطولة</button></div><div class="agh-team-directory">${list.map(x => `<button type="button" class="agh-entity-row" data-route="team/${esc(x.id)}"><i>${img(x.logo_url,x.name)}</i><span><b>${esc(x.name)}</b><small>${x.group_name ? `المجموعة ${esc(x.group_name)} · ` : ''}${x.coach ? `المدرب ${esc(x.coach)}` : esc(t.short_name || '')}</small></span>${shirt(x)}<em>${store.players.filter(p => p.team_id === x.id).length} لاعب</em><strong>‹</strong></button>`).join('')}</div></section>`).join('') || '<div class="agh-dir-empty">لا توجد فرق في هذا القسم.</div>'}`;
